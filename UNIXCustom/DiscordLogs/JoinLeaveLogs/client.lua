TriggerServerEvent('FRG:joinLog', source)
    ...
    while true do
        ...
        if IsEntityDead(playerPed) and not alreadyDead then
            ...
            if checkArray (Melee, death) then
                TriggerServerEvent('playerDied', tostring(killername) .. " meleed " .. playerName)
            elseif checkArray (Bullet, death) then
                TriggerServerEvent('playerDied', tostring(killername) .. " shot " .. playerName)
            elseif checkArray (Knife, death) then
                TriggerServerEvent('playerDied', tostring(killername) .. " stabbed " .. playerName)
            elseif checkArray (Car, death) then
                TriggerServerEvent('playerDied', tostring(killername) .. " hit " .. playerName)
